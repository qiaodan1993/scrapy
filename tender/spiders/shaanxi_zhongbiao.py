...
script = """
         function main(splash, args)
            ...
            js = string.format("document.getElementById('noticetype_5').click();document.getElementById('infoNoticeInputPage').value=%s;document.getElementsByClassName('btn btn-bg')[1].click()",args.pageNum)
            splash:evaljs(js)
            assert(splash:wait(args.wait))
            return splash:html()
         end
         """
class ShaanxiZhongbiaoSpider(scrapy.Spider):
    name = 'shaanxi_zhongbiao'
